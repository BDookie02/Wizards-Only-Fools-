...
import { ChapelInterior, getChapelInteriorSummary } from "./survivalGraveyardChapelInterior";
import {
  ChapelCeiling,
  ChapelDoubleDoor,
  ChapelGargoyle,
  ChapelWatchTower,
  getChapelExteriorPartsSummary,
...
export function getGraveyardChapelViewSummary() {
  const exterior = getChapelExteriorPartsSummary();
  const interior = getChapelInteriorSummary();
  const frontWindowCount = 1 + CHAPEL_SIDE_SIGNS.length;
  const sideWindowCount = CHAPEL_SIDE_SIGNS.length * (CHAPEL_SIDE_WING_WINDOW_Z.length + CHAPEL_NAVE_WINDOW_Z.length);
  const rearWindowCount = 1;

  return {
    ...exterior,
    ...interior,
    foundationCount: 1 + CHAPEL_SIDE_SIGNS.length,
    doorCount: CHAPEL_EXIT_RAMP_DEFINITIONS.length,
    exitRampCount: CHAPEL_EXIT_RAMP_DEFINITIONS.length,
    exitShadowCount: CHAPEL_EXIT_SHADOW_DEFINITIONS.length,
    windowCount: frontWindowCount + sideWindowCount + rearWindowCount,
    buttressCount: CHAPEL_SIDE_SIGNS.length * (CHAPEL_WING_BUTTRESS_Z.length + CHAPEL_CENTRAL_BUTTRESS_Z.length),
    towerPierCount: CHAPEL_SIDE_SIGNS.length * 3 + 2,
  };
}

export function GraveyardCatholicChapel({ baseHeight, showDetails }: { baseHeight: number; showDetails: boolean }) {
  const roof = "#171319";
