...
import {
  getCachedSurvivalTutorialGrassStrandGeometry,
  getSurvivalTutorialGrassBatchBladeGeometry,
  getSurvivalTutorialGrassBatchFlowerInstances,
} from "./survivalTutorialGrassCache";
import {
  makeSurvivalTutorialGrassFallbackTufts,
...
import {
  getSurvivalTutorialGrassBatchBounds,
  getSurvivalTutorialGrassCellBounds,
  uploadSurvivalTutorialGrassTufts,
} from "./survivalTutorialGrassInstancing";
...
function SurvivalTutorialGrassBatchTile({
  ...
  const batchBounds = useMemo(() => getSurvivalTutorialGrassBatchBounds(batch.cells), [batchSignature]);
  const bladeGeometry = useMemo(
    () => getSurvivalTutorialGrassBatchBladeGeometry(batch.cells),
    [batchSignature],
  );
...
function SurvivalTutorialGrassCellTile({
  ...
  const tufts = useMemo<SurvivalTutorialGrassTuft[]>(
    () => makeSurvivalTutorialGrassFallbackTufts(cell),
    [cell],
  );
  const cellBounds = useMemo(() => getSurvivalTutorialGrassCellBounds(cell), [cell]);

  const localFlowers = useMemo<SurvivalWildflower[]>(
    () => makeSurvivalTutorialGrassFlowerInstances(cell, mobilePerformanceMode),
    [cell, mobilePerformanceMode],
  );
  ...
  useEffect(() => {
    ...
    uploadSurvivalTutorialGrassTufts(mesh, cell, tufts, dummy, normal);
  }, [cell, dummy, normal, tufts]);
  ...
  useEffect(() => {
    ...
    uploadSurvivalFlowerInstances({
      ...
      dummy,
      normal,
      boundsX: cellBounds.x,
      boundsZ: cellBounds.z,
      boundsRadius: cellBounds.radius,
      boundsPadding: 18,
      baseX: cell.x,
      baseZ: cell.z,
    });
  }, [bellFlowers, cell.x, cell.z, cellBounds, dummy, localFlowers, normal, puffFlowers, roundFlowers, starFlowers]);
