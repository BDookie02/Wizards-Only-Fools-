...
const AVATAR_WORLD_HEIGHT = 2.95;
const AVATAR_WORLD_WIDTH = AVATAR_WORLD_HEIGHT;
const AVATAR_WORLD_CENTER_Y = 0.62;
...
function drawArm(
  ...
  const steps = Math.max(3, Math.ceil(Math.hypot(dx, dy) / 4));
...
function drawSlideDust(ctx: CanvasRenderingContext2D, frame: number, directionSign: number) {
  ...
  const dust = [
    { x: baseX - sign * (6 + pulse), y: 57, size: 2, alpha: 0.36 },
    { x: baseX - sign * (12 + pulse * 2), y: 60, size: 3, alpha: 0.28 },
    { x: baseX - sign * (18 + pulse), y: 55, size: 2, alpha: 0.22 },
    { x: baseX - sign * (24 + pulse * 2), y: 61, size: 1, alpha: 0.18 },
  ];

  dust.forEach((particle) => {
    drawBlock(ctx, particle.x, particle.y, particle.size + (pulse % 2), particle.size, `rgba(206, 179, 130, ${particle.alpha})`);
  });
}
...
function drawJumpTrail(ctx: CanvasRenderingContext2D, frame: number) {
  const pulse = frame % 4;
  const motes = [
    { x: 23, y: 57 + (pulse % 2), w: 2, h: 3, color: "rgba(165, 220, 255, 0.42)" },
    { x: 31, y: 59 - (pulse % 2), w: 2, h: 2, color: "rgba(255, 255, 255, 0.34)" },
    { x: 39, y: 57 + ((pulse + 1) % 2), w: 2, h: 3, color: "rgba(165, 220, 255, 0.38)" },
    { x: 28, y: 62, w: 1, h: 2, color: "rgba(130, 185, 255, 0.3)" },
    { x: 36, y: 62, w: 1, h: 2, color: "rgba(130, 185, 255, 0.3)" },
  ];

  motes.forEach((mote) => drawBlock(ctx, mote.x, mote.y, mote.w, mote.h, mote.color));
}
...
function getAvatarDirection(playerYaw: number, worldPosition: THREE.Vector3, camera: THREE.Camera) {
  ...
  useEffect(() => {
    ...
    const clearBlinkTimers = () => {
      if (blinkStartTimeoutRef.current !== null) {
        window.clearTimeout(blinkStartTimeoutRef.current);
        blinkStartTimeoutRef.current = null;
      }
      if (blinkEndTimeoutRef.current !== null) {
        window.clearTimeout(blinkEndTimeoutRef.current);
        blinkEndTimeoutRef.current = null;
      }
    };
    const scheduleBlink = () => {
      blinkStartTimeoutRef.current = window.setTimeout(() => {
        if (cancelled) return;
        setIsBlinking(true);
        blinkEndTimeoutRef.current = window.setTimeout(() => {
          if (cancelled) return;
          setIsBlinking(false);
          scheduleBlink();
        }, 95 + Math.random() * 70);
