...
function FoliageDodeca({
  ...
  return (
    <group position={position} scale={scale}>
      <mesh scale={[1.07, 1.065, 1.07]} castShadow={false}>
    ...
    meshes.forEach((mesh, colorIndex) => {
      ...
      blobs.forEach((blob) => {
        ...
        if (edgeMesh) {
          ...
          dummy.scale.set(blob.width * 1.055, blob.height * 1.075, blob.depth * 1.055);
          dummy.updateMatrix();
          edgeMesh.setMatrixAt(instance, dummy.matrix);
        }
  ...
  return (
    <group name={`survival-bushes-${chunk.key}`}>
      <instancedMesh ref={bushEdgeRef0} args={[undefined, undefined, capacity]}>
        ...
        <meshBasicMaterial color={PLANT_EDGE_COLOR} side={THREE.BackSide} />
      </instancedMesh>
    ...
    canopyMeshes.forEach((mesh, colorIndex) => {
      ...
      trees.forEach((tree) => {
        ...
        if (edgeMesh) {
          ...
          dummy.scale.set(tree.canopyRadius * 1.31, tree.canopyHeight * 1.06, tree.canopyRadius * 1.06);
          dummy.updateMatrix();
          edgeMesh.setMatrixAt(instance, dummy.matrix);
        }
    ...
    sideMeshes.forEach((mesh, colorIndex) => {
      ...
      trees.forEach((tree) => {
        ...
        if (edgeMesh) {
          ...
          dummy.scale.set(tree.canopyRadius * 0.82, tree.canopyHeight * 0.82, tree.canopyRadius * 0.68);
          dummy.updateMatrix();
          edgeMesh.setMatrixAt(instance, dummy.matrix);
        }
        dummy.position.set(
  ...
  return (
    <group name={`survival-fast-groves-${chunk.key}`}>
      ...
      <instancedMesh ref={canopyEdgeRef0} args={[undefined, undefined, capacity]}>
        ...
        <meshBasicMaterial color={PLANT_EDGE_COLOR} side={THREE.BackSide} transparent={canopyOpacity < 1} opacity={canopyOpacity} />
      </instancedMesh>
      ...
      <instancedMesh ref={sideCanopyEdgeRef0} args={[undefined, undefined, capacity]}>
        <dodecahedronGeometry args={[1, 0]} />
        <meshBasicMaterial color={PLANT_EDGE_COLOR} side={THREE.BackSide} transparent={canopyOpacity < 1} opacity={canopyOpacity * 0.96} />
      </instancedMesh>
    ...
    topMeshes.forEach((mesh, colorIndex) => {
      ...
      trees.forEach((tree) => {
        ...
        if (edgeMesh) {
          ...
          dummy.scale.set(tree.canopyWidth * 1.04, tree.canopyThickness * 1.1, tree.canopyDepth * 1.04);
          dummy.updateMatrix();
          edgeMesh.setMatrixAt(instance, dummy.matrix);
        }
    ...
    lowerMeshes.forEach((mesh, colorIndex) => {
      ...
      trees.forEach((tree) => {
        ...
        if (edgeMesh) {
          ...
          dummy.scale.set(tree.canopyWidth * 0.75, tree.canopyThickness * 0.8, tree.canopyDepth * 0.75);
          dummy.updateMatrix();
          edgeMesh.setMatrixAt(instance, dummy.matrix);
        }
        dummy.position.set(
      ...
      {Array.from({ length: 9 }, (_, index) => {
        ...
        return (
          <group key={index} position={[Math.sin(angle) * 4.2, 25.2, Math.cos(angle) * 4.2]} rotation={[0.5, angle, 0.18]}>
            <mesh scale={[1.08, 1.1, 1.04]} castShadow={false}>
...
function ChapelChandelier({ z }: { z: number }) {
  ...
  return (
    <group position={[0, 23.4, z]}>
      ...
      {candlePositions.map(([x, y, zOffset], index) => (
...
type MountainMineshaftPlatformPiece = {
  key: string;
  centerX: number;
  width: number;
};
...
function makeMountainVillageTrailSegments(
  ...
  return points.slice(0, -1).map((point, index) => {
    ...
    const supports = [-1, 1].map((side) => {
      ...
      const topY = midpoint.y - 1.18;
      const groundY = getMountainVillageHeight(chunk, localX, localZ, baseHeight) + 0.22;
      const height = topY - groundY;
      if (height < 4.6) return null;
    ...
    }).filter((support): support is MountainVillageTrailSupport => support !== null);
...
function MountainHutWallDetails({
  ...
  return (
    <>
      {Array.from({ length: frontPlankCount }, (_, index) => {
      ...
      {[-1, 1].map((side) => (
        <Fragment key={`side-wall-detail-${side}`}>
          {Array.from({ length: sidePlankCount }, (_, index) => {
          ...
          <group key={segment.key} position={[segment.localX, segment.y, segment.localZ]} rotation={[segment.slope, segment.yaw, 0]}>
            ...
            {showDetails && (
              <>
                ...
                {Array.from({ length: 4 }, (_, plankIndex) => {
...
function getMountainMineshaftLadderLandingLocalX(hut: MountainMineshaftHut, ladder?: MountainMineshaftLadder) {
...
function getMountainMineshaftPlatformPieces(width: number, gapCenterX: number | null, gapWidth: number): MountainMineshaftPlatformPiece[] {
  if (gapCenterX === null) {
  ...
  const gapMin = THREE.MathUtils.clamp(gapCenterX - halfGap, -halfWidth, halfWidth);
  const gapMax = THREE.MathUtils.clamp(gapCenterX + halfGap, -halfWidth, halfWidth);
    ...
    <group position={[hut.localX, hut.y, hut.localZ]} rotation={[0, hut.rotation, 0]}>
      ...
      {showDetails && platformPieces.map((piece) => (
        <Fragment key={`platform-detail-${piece.key}`}>
          ...
          {Array.from({ length: platformPlankCount }, (_, plankIndex) => {
    ...
    <group position={[ladder.localX, ladder.startY, ladder.localZ]} rotation={[0, ladder.rotation, 0]}>
      ...
      {showDetails && (
        <>
          ...
          {Array.from({ length: Math.min(10, Math.floor(rungCount / 2)) }, (_, index) => {
          ...
          {Array.from({ length: Math.min(12, Math.floor(rungCount / 2)) }, (_, index) => {
